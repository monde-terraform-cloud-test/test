...
resource "aws_security_group_rule" "alb_role_https" {
  for_each = {
    ...
    edion_test_client         = local.source_ip_address_edion_test_client
    edion_Exchange_Dataspider = local.source_ip_address_edion_Exchange_Dataspider
    cloudflare_ip             = local.source_ip_address_cloudflare_ip
    # nat_gateway               = local.source_ip_address_nat_gateway
  }
  type        = "ingress"
