  ...
  container_definitions = jsonencode([
    {
      ...
      "environmentFiles" : [
        {
          "value" : "arn:aws:s3:::${local.resource_prefix}-config/${var.role}/develop.env",
          "type" : "s3"
        }
      ],
      "dockerLabels" : {
        "com.datadoghq.tags.env": "${local.env}",
        "com.datadoghq.tags.service": "${var.role}",
      },
      "portMappings" : [
        {
          "hostPort" : 8080,
    ...
    },
    {
      ...
      "secrets" = [
        {
          "name" : "DD_API_KEY",
          "valueFrom" : "arn:aws:secretsmanager:ap-northeast-1:846364808165:secret:DdApiKeySecret-LDbktqYTCKg4-yHVOzK"
        },
      ]
      "dockerLabels" : {
        "com.datadoghq.tags.env": "${local.env}",
        "com.datadoghq.tags.service": "datadog-${var.role}",
      },
      "logConfiguration" : {
        "logDriver" : "awslogs",
        "options" : {
