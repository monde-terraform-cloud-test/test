...
locals {
  ...
  service_domain          = ""
  vpc_cidr                = "10.124.0.0/16" # NEW_EC_INFRA-17
  stg_vpc_cidr            = "10.125.0.0/16" # NEW_EC_INFRA-17
  dev_vpc_cidr            = "10.116.56.0/21" # NEW_EC_INFRA-17
  ...
  source_ip_address_nat_gateway = [
    ...
    "${data.aws_nat_gateway.main["public_c"].public_ip}/32",
  ]
  ...
  source_ip_address_edion_dns_server01 = "10.68.222.1/32" # NEW_EC_INFRA-66
  source_ip_address_edion_dns_server02 = "10.70.222.1/32" # NEW_EC_INFRA-66

  domains = {
    #private = "${var.project_name}.${local.env}.local"
    private = "edionsys1.internal"
    service = "edion.com"
  }
...
variable "transit_gateway" {
  ...
  default = {
    ...
    "system_cloud_gateway_route_table_id" = "tgw-rtb-xxxxxxxxxxxxxxxx" # transit gateway接続元アカウントで作成するリソースのため不要
  }
}